...
//디버깅용
//var losttime = 30; // 실종 경과 시간
//var targetspeed = 1.2; //아이의 평균 보행속도(m/s)
//var lostposition = { "latitude": 37.293884022802985, "longitude": 126.97476726025341 };
...
function find_nearest_node(lostposition, losttime, targetspeed) {
    var min = 10000000;
    var dist = 0, minindex, mindistance;
    ...
    nearestPosition = { "latitude": "0", "longitude": "0" };
    nearestPosition.latitude = graph.node[minindex].latitude;
    nearestPosition.longitude = graph.node[minindex].longitude;

    mindistance = computeDistance(nearestPosition, lostposition) * 1000;
    losttime = losttime - mindistance / targetspeed;
    //console.log("in find nearest : ", losttime);
    return [minindex, losttime];
}
...
function mainfucntions(losttime, targetspeed, lostposition) {
    ...
    var nearest = find_nearest_node(lostposition, losttime, targetspeed);
    if (nearest[1] <= 0) {
        for (i = 0; i < MARK; i++) {
            returnval.node[i].id = 1000;
            returnval.node[i].latitude = parseFloat(lostposition.latitude);
            returnval.node[i].longitude = parseFloat(lostposition.longitude);
        }
        return returnval;
    }
    else {
        next_nodes.push([nearest[0], nearest[0]]); // 최초 노드(center index == before index)
